    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs">
        ...
        <form id="poll-show-form" class="mt-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                ?>
                    <label class="candidate-card relative flex items-center p-3.5 sm:p-4 rounded-2xl border-2 <?= $isSelected ? 'border-emerald-600 bg-emerald-50/80 ring-2 ring-emerald-500/20' : 'border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/40' ?> cursor-pointer transition-all group has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50/80 has-[:checked]:ring-2 has-[:checked]:ring-emerald-500/20 shadow-2xs">
                        ...
                        <div class="ml-3 flex-1 min-w-0 flex items-center justify-between">
                            <div>
                                <span class="font-extrabold text-slate-900 text-sm sm:text-base truncate block"><?= htmlspecialchars($opt['name']) ?></span>
                                <p class="text-xs text-slate-500 font-medium truncate mt-0.5"><?= htmlspecialchars($opt['party']) ?></p>
                            </div>
                            <span class="w-3.5 h-3.5 rounded-full border border-slate-300 shrink-0 ml-2" style="background-color: <?= htmlspecialchars($opt['avatarColor']) ?>;"></span>
                        </div>
                    </label>
                <?php endforeach; ?>
            </div>

            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                ...
                <button type="submit" id="poll-show-btn" class="bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold text-sm px-8 py-3 rounded-xl shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 group cursor-pointer">
                ...
                <div class="option-row p-3 rounded-xl <?= $isWinner ? 'bg-amber-50/50 border border-amber-200/80 shadow-2xs' : 'bg-slate-50/50' ?>" data-id="<?= htmlspecialchars($res['optionId']) ?>">
                    <div class="flex items-center justify-between text-xs font-bold mb-1.5">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-slate-900 font-black text-sm option-pct-label"><?= number_format($res['percentage'], 1) ?>% <span class="text-slate-400 font-normal text-xs option-votes-label">(<?= number_format($res['votes']) ?> votes)</span></span>
                        </div>
                    </div>
