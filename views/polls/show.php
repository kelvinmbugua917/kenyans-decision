...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <!-- Poll Header -->
    <div class="bg-white rounded-2xl p-4 sm:p-8 border border-slate-200 shadow-xs">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-3">
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs">
                <a href="/polls/<?= htmlspecialchars($poll['id']) ?>/history" class="inline-flex items-center gap-1.5 font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded-lg border border-slate-300 transition-colors">
                    ...
                    <span>Results History & Momentum</span>
                </a>
                <a href="https://api.whatsapp.com/send?text=<?= urlencode('I just voted on Kenyans Decision: "' . ($poll['title'] ?? '') . '". See live results and vote: ' . (($_SERVER['HTTPS'] ?? 'off') === 'on' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . '/polls/' . ($poll['id'] ?? '')) ?>" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 font-bold text-emerald-800 bg-emerald-100 hover:bg-emerald-200 px-2.5 py-1 rounded-lg border border-emerald-300 transition-colors">
        ...
        <!-- Voting Form -->
        <form id="poll-show-form" class="mt-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                ?>
                    <label class="candidate-card relative flex items-center p-3 sm:p-4 rounded-2xl border-2 <?= $isSelected ? 'border-emerald-600 bg-emerald-50/80 ring-2 ring-emerald-500/20' : 'border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/40' ?> cursor-pointer transition-all group has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50/80 has-[:checked]:ring-2 has-[:checked]:ring-emerald-500/20 shadow-2xs">
                        <input type="radio" name="optionId" value="<?= htmlspecialchars($opt['id']) ?>" <?= $isSelected ? 'checked' : '' ?> class="w-4 h-4 text-emerald-600 focus:ring-emerald-500 border-slate-300 shrink-0">
                        <div class="ml-2.5 sm:ml-3 flex-1 min-w-0 flex items-center justify-between">
                            <div class="min-w-0 pr-1">
                                <span class="font-extrabold text-slate-900 text-xs sm:text-base truncate block"><?= htmlspecialchars($opt['name']) ?></span>
                                <p class="text-[11px] sm:text-xs text-slate-500 font-medium truncate mt-0.5"><?= htmlspecialchars($opt['party']) ?></p>
                            </div>
                            <span class="w-3.5 h-3.5 rounded-full border border-slate-300 shrink-0 ml-1.5" style="background-color: <?= htmlspecialchars($opt['avatarColor']) ?>;"></span>
                        </div>
                    </label>
                <?php endforeach; ?>
            </div>

            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-3 bg-slate-50 p-3.5 sm:p-4 rounded-2xl border border-slate-200">
                <div class="flex items-center justify-between sm:justify-start gap-2">
                    <label class="text-xs font-bold text-slate-700 shrink-0">County:</label>
                ...
                <button type="submit" id="poll-show-btn" class="bg-emerald-700 hover:bg-emerald-800 text-white font-extrabold text-xs sm:text-sm px-6 py-2.5 sm:py-3 rounded-xl shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 group cursor-pointer w-full sm:w-auto">
                    <svg class="w-4 h-4 text-white shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <span><?= !empty($hasVoted) ? 'Change Vote Choice' : 'Cast Vote' ?></span>
                </button>
            </div>
        </form>
    ...
    <?php 
        $maxVotes = 0;
        foreach ($results['optionResults'] as $r) {
            if ($r['votes'] > $maxVotes) {
                $maxVotes = $r['votes'];
            }
        }
    ?>
    <div class="bg-white rounded-2xl p-4 sm:p-8 border border-slate-200 shadow-xs space-y-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 border-b border-slate-200">
            <div>
                <h2 class="text-base sm:text-lg font-bold text-slate-900">National & County Poll Standings</h2>
                <p class="text-xs text-slate-500">Filter standings by specific Kenyan county or view national totals.</p>
            </div>

            <div class="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-start">
                <label class="text-xs font-bold text-slate-700 shrink-0">Filter Breakdown:</label>
                <select id="county-filter" class="text-xs font-bold bg-slate-100 border border-slate-300 rounded-lg px-2.5 py-1.5 focus:ring-2 focus:ring-emerald-500 max-w-[200px] sm:max-w-none">
        ...
        <div id="results-container" class="space-y-3.5">
            ...
            foreach ($results['optionResults'] as $idx => $res): 
                $isWinner = ($res['votes'] > 0 && $res['votes'] === $maxVotes);
                $trend = $trendsMap[$idx % 4];
            ?>
                <div class="option-row p-3 rounded-xl <?= $isWinner ? 'bg-amber-50/50 border border-amber-200/80 shadow-2xs' : 'bg-slate-50/50' ?>" data-id="<?= htmlspecialchars($res['optionId']) ?>">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 text-xs font-semibold mb-1.5">
                        ...
                        <div class="flex items-center justify-between sm:justify-end gap-2 shrink-0">
                            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded border <?= $trend['class'] ?>"><?= $trend['badge'] ?> (Since launch)</span>
                            <span class="text-slate-900 font-black text-xs sm:text-sm option-pct-label"><?= number_format($res['percentage'], 1) ?>% <span class="text-slate-500 font-normal text-xs option-votes-label">(<?= number_format($res['votes']) ?> votes)</span></span>
                        </div>
                    </div>
